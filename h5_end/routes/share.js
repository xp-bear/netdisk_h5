...
router.post('/files/:id', async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: '分享文件失败' })
  }
})
...
router.delete('/files/:id', async (req, res) => {
  try {
    ...
    console.error('取消分享文件出错:', error)
    res.status(500).json({ success: false, message: '取消分享文件失败' })
  }
})
...
router.get('/square', async (req, res) => {
  try {
    ...
    console.error('获取分享文件出错:', error)
    res.status(500).json({ success: false, message: '获取分享文件失败' })
  }
})
